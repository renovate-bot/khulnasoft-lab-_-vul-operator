...
// PluginContext is plugin's execution context within the Vul-operator toolkit.
...
type PluginContext interface {
	...
	// GetNamespace return the name of the K8s Namespace where Vul-operator creates Jobs
	// and other helper objects.
	GetNamespace() string
	// GetServiceAccountName return the name of the K8s Service Account used to run workloads
	// created by Vul-operator.
	GetServiceAccountName() string
	// GetVulOperatorConfig returns vuloperator configuration.
	GetVulOperatorConfig() ConfigData
}
...
func GetPluginConfigMapName(pluginName string) string {
	return "vul-operator-" + strings.ToLower(pluginName) + "-config"
}
...
func (p *pluginContext) EnsureConfig(config PluginConfig) error {
	err := p.client.Create(context.Background(), &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: labels.Set{
				LabelK8SAppManagedBy: "vuloperator",
			},
		},
		Data: config.Data,
	})
...
func (p *pluginContext) GetVulOperatorConfig() ConfigData {
	return p.vulOperatorConfig
}
...
func (b *PluginContextBuilder) WithVulOperatorConfig(config ConfigData) *PluginContextBuilder {
	b.ctx.vulOperatorConfig = config
	return b
}
