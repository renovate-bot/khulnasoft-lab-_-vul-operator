...
var (
	...
	IMAGE_TAG                 = "dev"
	VUL_OPERATOR_IMAGE      = "khulnasoft-lab/vul-operator:" + IMAGE_TAG
	VUL_OPERATOR_IMAGE_UBI8 = "khulnasoft-lab/vul-operator:" + IMAGE_TAG + "-ubi8"
...
// Target for building vul-operator binary.
func (b Build) Binary() error {
	fmt.Println("Building vul-operator binary...")
	return sh.RunWithV(LINUX_ENV, "go", "build", "-o", "./bin/vul-operator", "./cmd/vul-operator/main.go")
}
...
// Target for running integration tests for Vul Operator.
func (t Test) Integration() error {
	fmt.Println("Running integration tests for Vul Operator...")
	mg.Deps(checkKubeconfig, getGinkgo)
	return sh.RunV(GINKGO, "-coverprofile=coverage.txt",
		"-coverpkg=github.com/khulnasoft-lab/vul-operator/pkg/operator,"+
			"github.com/khulnasoft-lab/vul-operator/pkg/operator/predicate,"+
			"github.com/khulnasoft-lab/vul-operator/pkg/operator/controller,"+
			"github.com/khulnasoft-lab/vul-operator/pkg/plugin,"+
			"github.com/khulnasoft-lab/vul-operator/pkg/plugin/vul,"+
...
// Target for building Docker image for vul-operator
func (b Build) Docker() error {
	fmt.Println("Building Docker image for vul-operator...")
	return sh.RunV("docker", "build", "--no-cache", "-t", VUL_OPERATOR_IMAGE, "-f", "build/vul-operator/Dockerfile", "bin")
}

// Target for building Docker image for vul-operator ubi8
func (b Build) DockerUbi8() error {
	fmt.Println("Building Docker image for vul-operator ubi8...")
	return sh.RunV("docker", "build", "--no-cache", "-f", "build/vul-operator/Dockerfile.ubi8", "-t", VUL_OPERATOR_IMAGE_UBI8, "bin")
}
...
func (b Build) KindLoadImages() error {
	...
	return sh.RunV(KIND, "load", "docker-image", VUL_OPERATOR_IMAGE, VUL_OPERATOR_IMAGE_UBI8)
}
...
func (d Docs) Serve() error {
	...
	err := sh.RunV("docker", "build", "-t", MKDOCS_IMAGE, "-f", "build/mkdocs-material/Dockerfile", "build/vul-operator")
	if err != nil {
		return err
	}
...
func (g Generate) Code() error {
	...
	return sh.RunV(CONTROLLER_GEN, "object:headerFile=hack/boilerplate.go.txt", "paths=./pkg/...", "+rbac:roleName=vul-operator", "output:rbac:artifacts:config=deploy/helm/generated")
}
