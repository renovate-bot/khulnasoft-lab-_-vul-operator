...
You also need the Vul-Operator to be installed in the `vul-system` namespace, e.g. with
[kubectl](./installation/kubectl.md) or [Helm](./installation/helm.md). Let's also assume that the operator is
configured to discover built-in Kubernetes resources in all namespaces, except `kube-system` and `vul-system`.
...
replicaset-nginx-78449c65d4-nginx   library/nginx   1.16   Vul     85s   33         62     49       114   1
```
</details>
...
replicaset-nginx-78449c65d4   Vul       2m7s      0         0      6        7
```
</details>
...
The reports will be deleted after 24 hours. When a VulnerabilityReport gets deleted Vul-Operator will automatically rescan the resource.
...
- By default, the operator uses Vul as [Vulnerability Scanner] and Polaris as [Configuration Checker], but you can
  choose other tools that are integrated with Vul-Operator or even implement you own plugin.
